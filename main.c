...
// External function from server.c
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1 && strcmp(argv[1], "--server") == 0) {
        printf("Starting web server mode...\n");
        return start_server();
    }
...
// Load data from files
void loadData() {
    FILE *bookFile = fopen("data/books.dat", "rb");
    FILE *memberFile = fopen("data/members.dat", "rb");
    FILE *transactionFile = fopen("data/transactions.dat", "rb");
    
    if (bookFile != NULL) {
        book_count = fread(books, sizeof(Book), 100, bookFile);
        fclose(bookFile);
    }
    
    if (memberFile != NULL) {
        member_count = fread(members, sizeof(Member), 50, memberFile);
        fclose(memberFile);
    }
    
    if (transactionFile != NULL) {
        transaction_count = fread(transactions, sizeof(Transaction), 200, transactionFile);
...
// Save data to files
void saveData() {
    FILE *bookFile = fopen("data/books.dat", "wb");
    FILE *memberFile = fopen("data/members.dat", "wb");
    FILE *transactionFile = fopen("data/transactions.dat", "wb");
    
    if (bookFile != NULL) {
        fwrite(books, sizeof(Book), book_count, bookFile);
        fclose(bookFile);
    } else {
        printf("Error: Could not save book data.\n");
    }
    
    if (memberFile != NULL) {
        fwrite(members, sizeof(Member), member_count, memberFile);
        fclose(memberFile);
    } else {
    ...
    if (transactionFile != NULL) {
        fwrite(transactions, sizeof(Transaction), transaction_count, transactionFile);
        fclose(transactionFile);
    } else {
